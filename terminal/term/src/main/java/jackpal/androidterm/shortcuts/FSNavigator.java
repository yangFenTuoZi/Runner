...
public class FSNavigator extends AppCompatActivity {
    ...
    private LinearLayout fileEntry(final String entry) {
        LinearLayout ll;
        if (entry == null) ll = fileView(true);
        else {
            if (countFileView < cachedFileView.size()) ll = cachedFileView.get(countFileView);
            else cachedFileView.put(countFileView, ll = fileView(false));
            ++countFileView;
        }
    ...
    private LinearLayout directoryView(boolean up) {
        ...
        if (up) tv.setGravity(Gravity.CENTER_VERTICAL | Gravity.START);//Gravity.CENTER);
        else tv.setGravity(Gravity.CENTER_VERTICAL | Gravity.START);
        tv.setClickable(true);
        tv.setLongClickable(true);
        tv.setFocusable(true);
    ...
    private void makeView() {
        ...
        ScrollView sv = contentView.findViewById(R.id.scrollview);
        LinearLayout ll = sv.findViewById(R.id.mainview);
        ll.removeAllViews();
        if (cd == null) chdir("/");
...
//    titleView.setSelected(true);
    ...
    java.util.Comparator<String> stringSortComparator = Comparator.comparing(String::toLowerCase);
