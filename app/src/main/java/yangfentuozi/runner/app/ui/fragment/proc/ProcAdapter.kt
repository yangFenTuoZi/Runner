package yangfentuozi.runner.app.ui.fragment.proc
        ...
        data = if (Runner.pingServer()) {
            try {
                ...
                if (processes == null) null
                    ...
                    for (pi in processes) {
                        if (pi.exe == ServerMain.USR_PATH + "/bin/bash") a.add(pi)
                    }
                    a
                }
    ...
    companion object {
        ...
        fun killPIDs(pids: IntArray) {
            if (Runner.pingServer()) {
                try {
                    val signal = if (App.Companion.preferences.getBoolean("force_kill", false)) 9 else 15
                    if (App.Companion.preferences.getBoolean("kill_child_processes", false)) {
                        val processes = Runner.service?.processes
                        if (processes == null) {
